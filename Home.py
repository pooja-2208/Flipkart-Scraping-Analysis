...
def home():
    ...
    if st.button('🚀Start Scraping'):
        try:
            ...
            data.to_sql('scraped_rawdata', con=engine, if_exists='replace', index=False)

            st.dataframe(data)

            query = "SELECT * FROM scraped_rawdata"
            ...
            print(raw_data.Brand.nunique())
            print(raw_data.dtypes)

            print(raw_data.loc[raw_data['Discount']<0, :])

            # Data Cleaning
            raw_data.columns=raw_data.columns.str.strip().str.lower().str.replace(' ', '_')
            raw_data.rename(columns={'index':'record_id'}, inplace=True)

            raw_data['price']=pd.to_numeric(raw_data['price'], errors='coerce')
            raw_data['rating']=pd.to_numeric(raw_data['rating'], errors='coerce')
            raw_data['number_of_ratings']=raw_data['number_of_ratings'].str.replace(',','')
            raw_data['number_of_ratings']=raw_data['number_of_ratings'].astype('Int64')
